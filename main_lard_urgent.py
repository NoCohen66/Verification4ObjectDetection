...
eps_urgent = np.linspace(0,0.002, 11)
#eps_urgent = np.linspace(0,0.002, 11)
#eps_urgent = np.linspace(0, 0.1,11)
for image_id in range(40):
    ...
    for method in ['IBP+backward (CROWN-IBP)']:
        print("method", method)
        for i in range(len(eps_urgent)):
            print("variations", eps_urgent[i])
            start_perturbation = time.time() 
            lb_box_wn, ub_box_wn = bound_whitenoise(torch_model, X, eps_urgent[i], method=method.split()[0])
            #lb_box_bri, ub_box_bri = bound_brightness_LARD(model_bri, X, eps_urgent[i], method=method.split()[0])
            ...
            #lb_box_contr, ub_box_contr = bound_contrast_LARD(model_contr, X, eps_urgent[i], method=method.split()[0])
    ...
    df.to_csv(f"urgent_new_results/LARD_IBPCROWN_whitenoise/{image_id}_iou_calculations.csv")
  

pd.DataFrame(list_info_time).to_csv("urgent_new_results/LARD_IBPCROWN_whitenoise/infos/times.csv")
pd.DataFrame(images_exp).to_csv("urgent_new_results/LARD_IBPCROWN_whitenoise/infos/images.csv")
