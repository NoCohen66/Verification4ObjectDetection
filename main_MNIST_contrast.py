...
list_info_time = []
images_exp = []
eps_list = np.linspace(0, 0.2,11)
#eps_list_contrast = np.linspace(0, 0.01,101)

for image_id in range(41):
    ...
    #for method in ['backward (CROWN)']:
        ...
        for i in range(len(eps_list)):
            ...
            lb_box_contr, ub_box_contr = bound_contrast(model_corners_contrast, X, eps_list[i], method=method.split()[0])
            ...
            #perturbations_dict = {"whitenoise": [lb_box_bri, ub_box_bri]}
    ...
    df.to_csv(f"urgent_new_results/MNIST_CROWN_contrast/{image_id}_iou_calculations.csv")
  

pd.DataFrame(list_info_time).to_csv("urgent_new_results/MNIST_CROWN_contrast/infos/times2.csv")
pd.DataFrame(images_exp).to_csv("urgent_new_results/MNIST_CROWN_contrast/infos/images.csv")
